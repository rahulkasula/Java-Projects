...
String[] x= {"rahul    testing    hiiii"};
        //GraphView graph = (GraphView) findViewById(R.id.graph);

        manageSchedule=(ListView)findViewById(R.id.listView);
         adapter2 =new ArrayAdapter<String>(this,android.R.layout.simple_list_item_1,android.R.id.text1,listItems);
manageSchedule.setAdapter(adapter);
    ...
    private class BackTask extends AsyncTask<Void,Void,Void> {
        ...
        protected Void doInBackground(Void...params){
            String getDetails_URL="http://10.0.0.9/Display.php";
            try {
...
//                System.out.print("part" + parts[1]);
        ...
        protected  void onPostExecute(Void result){
            ...
            manageSchedule.setAdapter(adapter2);
    ...
    private class BackTask2 extends AsyncTask<Void,Void,ArrayList<VSBeans>> {
        ...
        protected ArrayList doInBackground(Void... params) {
            ...
            String getDetails_URL = "http://10.0.0.9/CustomerGraph2.php";
            try {
                URL url = new URL(getDetails_URL);
                HttpURLConnection httpURLConnection = (HttpURLConnection) url.openConnection();
                httpURLConnection.setRequestMethod("POST");
                ...
                try {
                    ...
                    for (int i = 0; i < arr.length(); i++) {
                        ...
                        listPro1.get(i).setApplianceName(arr.getJSONObject(i).getString("appliance"));
                    }
        ...
        protected void onPostExecute(ArrayList<VSBeans> result) {
            ...
            for(int i=0,j=0;i<(result.size()-1);i++){
                ...
                y[i][0]=new  DataPoint(Integer.parseInt(result.get(i).getStart()),Double.parseDouble(result.get(i).getPowerconsumption()));
                y[i][1]=new  DataPoint(Integer.parseInt(result.get(i).getEnd()),Double.parseDouble(result.get(i).getPowerconsumption()));
//gv[i].setTitle(result.get(i).getApplianceName());
            }
            ...
            for(int i=0,j=0;i<(result.size()-1)&&j<2*((result.size()-1));i++){
                ...
                x[j]=new  DataPoint(Integer.parseInt(result.get(i).getStart()),Double.parseDouble(result.get(i).getPowerconsumption()));
                x[j+1]=new DataPoint(Integer.parseInt(result.get(i).getEnd()),Double.parseDouble(result.get(i).getPowerconsumption()));
                j=j+2;
            }
