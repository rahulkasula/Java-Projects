 ...
 */
package edu.uncc.weadvise.usermanagement.servlet;
...
public class UserLogin extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        User user = WebServiceFacade.getUser(username);
        System.out.println("@@User returned : "+user);
        if(user != null && user.getPassowrd().equals(password)) {
            request.getSession().setAttribute("user", user);
            getServletContext().getRequestDispatcher("/home.jsp").forward(request, response);
        } else {
            getServletContext().getRequestDispatcher("/login.jsp?status=1").forward(request, response);
        }
    }
